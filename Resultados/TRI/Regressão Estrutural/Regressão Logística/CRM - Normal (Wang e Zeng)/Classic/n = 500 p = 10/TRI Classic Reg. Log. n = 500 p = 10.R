...
Dados.dir.Reg = file.path(main.dir, 'Dados Simulados', 'Regressão', 'Regressão Logística', paste0('n = ',
                      n))
Results.dir = file.path(main.dir, 'Resultados', 'TRI', 'Regressão Estrutural', 'Regressão Logística', 'CRM - Normal (Wang e Zeng)', 'Classic', paste0('n = ',
                    n,' p = ', p))
...
for (m in Contador) {
  ...
  # Lendo os dados
  Y = as.data.frame(read.table(paste0(Dados.dir.TRI, "/DadosTRI_", m, ".txt")))
...
for (m in 1:r) {
  for (i in 1:n) {
    EQM_theta[m, i] = as.numeric((theta_estimados_EM[i, m]) - thetas_reais[[3]][i])^2
  }
}
