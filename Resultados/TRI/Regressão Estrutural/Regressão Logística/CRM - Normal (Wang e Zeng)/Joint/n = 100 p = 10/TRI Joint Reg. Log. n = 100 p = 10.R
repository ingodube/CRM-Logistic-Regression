...
Dados.dir.TRI = file.path(main.dir, 'Dados Simulados', 'TRI', 'Modelo de Mensuração', 'CRM - Normal (Wang e Zeng)', 'Geral', paste0('n = ',
                      n,' p = ', p))
Dados.dir.Reg = file.path(main.dir, 'Dados Simulados', 'Regressão', 'Regressão Logística', paste0('n = ',
                      n))
Results.dir = file.path(main.dir, 'Resultados', 'TRI', 'Regressão Estrutural', 'Regressão Logística', 'CRM - Normal (Wang e Zeng)', 'Joint', paste0('n = ',
                    n,' p = ', p))
...
for (m in 1:r) {
  ...
  set.seed(49989+ m) # Variando as seeds
  ...
  #write.table(
    #resultados,
    #file = file.path(Dados.dir.TRI, paste0("dadosTRI_", m, ".txt")),
    #row.names = FALSE,
    #quote = FALSE,
    #dec = ".",
    #col.names = FALSE
...
for (m in 1:r) {
  ...
  #write.table(w, file = file.path(Dados.dir.Reg, paste0("Dados reg.log.", m, ".txt")),
...
for (m in Contador){
  set.seed(29111 + m)
  Y = read.table(paste0(Dados.dir.TRI, "/DadosTRI_", m, ".txt"), quote="\"", comment.char="")
  Z = log(Y/(k - Y))
  W = read.table(paste0(Dados.dir.Reg , "/Dados reg.log.", m, ".txt"), quote="\"", comment.char="")
  W = W$V1
...
mcmc_dens(amostras_jags, pars = "theta[57]")

beta_1_estimados_mean
max(beta_1_estimados_mean)
...
for (i in 1:n) {
  ...
  Vício_theta[i] = theta.est.médio[i] - thetas_reais[[1]][i]
}
...
for (m in 1:r) {
  for (i in 1:n) {
    EQM_theta[m, i] = as.numeric((theta_estimados_mean[i, m]) - thetas_reais[[1]][i])^2
  }
}
...
RMSEA_alpha = rep(0, p)
for (j in 1:p) {
  RMSEA_a[j] = sqrt(mean(EQM_a[, j]))
  RMSEA_b[j] = sqrt(mean(EQM_b[, j]))
  RMSEA_alpha[j] = sqrt(mean(EQM_alpha[, j]))
}

RMSEA_TRI = cbind(RMSEA_a, RMSEA_b, RMSEA_alpha)
