...
for (m in 1:r) {
  ...
  set.seed(121182 + m) # Variando as seeds
...
for (i in 1:n) {
  ...
  Vício_theta[i] = theta.est.médio[i] - thetas_reais[[3]][i]
}
...
for (m in 1:r) {
  for (i in 1:n) {
    EQM_theta[m, i] = as.numeric((theta_estimados_mean[i, m]) - thetas_reais[[3]][i])^2
  }
}
