...
for (m in 1:r) {
  ...
  set.seed(33429 + m) # Variando as seeds
...
round(beta_1_estimados_mean, 6)

median(beta_1_estimados_mean)
...
for (i in 1:n) {
  ...
  Vício_theta[i] = theta.est.médio[i] - thetas_reais[[2]][i]
}
...
for (m in 1:r) {
  for (i in 1:n) {
    EQM_theta[m, i] = as.numeric((theta_estimados_mean[i, m]) - thetas_reais[[2]][i])^2
  }
}
