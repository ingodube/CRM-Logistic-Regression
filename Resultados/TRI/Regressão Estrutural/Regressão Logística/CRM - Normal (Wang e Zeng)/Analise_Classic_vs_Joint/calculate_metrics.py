...
OUT = ROOT / "Analise_Classic_vs_Joint"
R_SCRIPT = "Rscript"
...
METHOD_FILES = {
    "Classic": {
        "a": "a_estimados_EM.txt",
        "b": "b_estimados_EM.txt",
        "alpha": "alpha_estimados_EM.txt",
        "theta": "theta_estimados_EM.txt",
    },
    "Joint": {
        "a": "a_estimados_mean.txt",
        "b": "b_estimados_mean.txt",
        "alpha": "alpha_estimados_mean.txt",
        "theta": "thetas_estimados_mean.txt",
    },
}
    ...
    for method, files in METHOD_FILES.items():
        ...
        for scenario_dir in sorted(base.glob("n = * p = *")):
            ...
            for param, filename in files.items():
                ...
                note = ""
                if status == "invalid":
                    note = "Dimensao ou valores invalidos; metrica marcada como NA."
                rows.append(
                    {
                        "n": n,
                        "p": p,
    ...
    rows.append(
        {
            ...
            "method": "Classic",
            "param": "theta",
            "file": "TRI Classic Reg. Log. *.R",
            "rows": None,
            "cols": None,
            "expected_rows": None,
            "expected_cols": None,
            "status": "warning",
            "note": "Os scripts Classic inspecionados gravam alpha_estimados_EM em theta_estimados_EM.txt; por isso cenarios com theta fora de n x 1000 ficam como NA.",
        }
    )
    return rows
...
def make_markdown(summary: list[dict[str, object]], theta_rows: list[dict[str, object]]) -> str:
    lines = [
        ...
        "## Resumo TRI",
        "",
        "| n | p | Metodo | RMSE theta | RMSE a | RMSE b | RMSE alpha | Vicio theta | Vicio a | Vicio b | Vicio alpha |",
        "|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "| n | p | Metodo | Media | DP | Min | Max | Amplitude |",
            "|---:|---:|---|---:|---:|---:|---:|---:|",
        ]
    )
                ...
                manual_checks.append(
                    {
                        ...
                        "bias": bias,
                        "rmse": rmse,
                        "note": "Cheque independente da orientacao: linha = item, coluna = replicacao.",
                    }
                )
