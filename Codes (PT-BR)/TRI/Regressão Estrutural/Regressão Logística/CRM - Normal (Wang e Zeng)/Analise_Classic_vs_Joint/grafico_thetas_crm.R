...
root_dir <- file.path(repo_root, "Results (PT-BR)", "TRI", "Regressão Estrutural", "Regressão Logística", "CRM - Normal (Wang e Zeng)")
output_dir <- file.path(root_dir, "Analise_Classic_vs_Joint")
spreadsheets_dir <- file.path(repo_root, "Spreadsheets (PT-BR)")
...
excel_path <- file.path(spreadsheets_dir, "estatisticas_thetas_estimados_crm.xlsx")
