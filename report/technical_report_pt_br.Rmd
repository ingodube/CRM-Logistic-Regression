...
affiliation: "Programa de Mestrado em Estatística e Ciência de Dados (PGECD/UFBA)"
secondary-affiliation: "Departamento de Economia Industrial (SENAI/CIMATEC)"
...
Na [Figura 2](#fig-crm-discriminacao), o aumento de $a_j$ torna as curvas progressivamente mais íngremes. O item 1, com $a_j=0{,}5$, apresenta transições graduais ao longo de uma faixa ampla de $\theta$, enquanto o item 4, com $a_j=3$, concentra a mudança de probabilidade em uma faixa estreita. Portanto, maiores valores de $a_j$ aumentam a capacidade do item de distinguir indivíduos com níveis próximos do traço latente e, consequentemente, elevam sua informação.
...
Na [Figura 3](#fig-crm-dificuldade), as curvas mantêm a mesma inclinação e o mesmo espaçamento, mas se deslocam horizontalmente à medida que $b_j$ aumenta. O item 1, com $b_j=-2$, requer níveis menores de $\theta$ para atingir determinada probabilidade acumulada; o item 4, com $b_j=2{,}5$, requer níveis maiores. Assim, $b_j$ indica a localização ou dificuldade do item na escala latente, sem alterar sua discriminação ou informação.
...
Na [Figura 4](#fig-crm-escala), valores menores de $\alpha_j$ produzem maior separação horizontal entre as curvas correspondentes aos pontos de corte $\lambda$, como ocorre no item 1, com $\alpha_j=0{,}5$. Conforme $\alpha_j$ aumenta, as curvas se aproximam e as transições entre níveis de resposta passam a ocorrer em uma região mais concentrada de $\theta$, comportamento mais evidente no item 4, com $\alpha_j=3$. Desse modo, $\alpha_j$ controla o espaçamento entre os limiares na escala do traço latente.
...
Na [Figura 5](#fig-crm-discriminacao-escala), a combinação de $a_j$ e $\alpha_j$ determina simultaneamente a inclinação e a separação das curvas. O item 2 combina baixa discriminação e baixo parâmetro de escala, resultando em curvas planas e muito espaçadas; o item 3 mantém a baixa discriminação, mas o maior $\alpha_j$ aproxima os limiares. Já o item 4 associa alta discriminação a baixo $\alpha_j$, produzindo curvas muito íngremes, porém separadas. Esses padrões mostram que os parâmetros exercem funções distintas e complementares na caracterização do item.
...
library(ggplot2)
beta_resultados = read.csv("../Spreadsheets (PT-BR)/beta_1_estimativas.csv",
                           check.names = FALSE)
...
tabela = read.csv("../Spreadsheets (PT-BR)/tabela_parametros.csv",
                  check.names = FALSE)
...
theta_resultados = read.csv("../Spreadsheets (PT-BR)/theta_medias.csv",
                            check.names = FALSE)
