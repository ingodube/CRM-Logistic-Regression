...
function buildSummary() {
  writeTitle(
    summarySheet,
    "Comparison Classic vs Joint - Parameters da IRT",
  ...
  summarySheet.getRange("D4:I4").values = [["RMSE", null, null, "Bias", null, null]];
...
function buildQuality() {
  writeTitle(
    qualitySheet,
    "Quality dos Data - Parameters da IRT",
    "Read dimensions for a, b, and alpha. The table excludes theta files.",
    11,
  );
...
await fs.writeFile(path.join(outDir, "inspect_parameters_tri.ndjson"), inspect.ndjson, "utf8");
...
await fs.writeFile(path.join(outDir, "inspect_parameters_tri_errors.ndjson"), errors.ndjson, "utf8");

for (const sheetName of ["IRT_Summary", "Quality"]) {
  const preview = await workbook.render({ sheetName, autoCrop: "all", scale: 1, format: "png" });
  await fs.writeFile(path.join(outDir, `Parameters_${sheetName}.png`), new Uint8Array(await preview.arrayBuffer()));
}

await fs.writeFile(path.join(outDir, "comparacao_parameters_tri.md"), buildMarkdown(), "utf8");

const output = await SpreadsheetFile.exportXlsx(workbook);
await output.save(path.join(outDir, "comparacao_parameters_tri_crm.xlsx"));
