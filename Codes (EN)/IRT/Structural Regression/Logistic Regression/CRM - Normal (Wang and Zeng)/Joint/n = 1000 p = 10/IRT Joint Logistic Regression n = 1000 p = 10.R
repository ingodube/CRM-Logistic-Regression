...
for (m in 1:r) {
  ...
  set.seed(229182 + m) # Vary random seeds
...
mcmc_dens(jags_samples, pars = "beta_1")

beta_1_estimates_mean
mean(beta_1_estimates_mean)
hist(beta_1_estimates_mean, main = NULL)
...
for (i in 1:n) {
  ...
  Bias_theta[i] = mean_theta_estimate[i] - true_thetas[[4]][i]
}
...
for (m in 1:r) {
  for (i in 1:n) {
    MSE_theta[m, i] = as.numeric((theta_estimates_mean[i, m]) - true_thetas[[4]][i])^2
  }
}
