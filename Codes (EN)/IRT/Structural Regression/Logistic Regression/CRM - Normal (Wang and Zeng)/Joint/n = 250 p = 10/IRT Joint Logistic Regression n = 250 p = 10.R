...
for (m in 1:r) {
  ...
  set.seed(33429 + m) # Vary random seeds
...
round(beta_1_estimates_mean, 6)

mean_valuen(beta_1_estimates_mean)
...
for (i in 1:n) {
  ...
  Bias_theta[i] = mean_theta_estimate[i] - true_thetas[[2]][i]
}
...
for (m in 1:r) {
  for (i in 1:n) {
    MSE_theta[m, i] = as.numeric((theta_estimates_mean[i, m]) - true_thetas[[2]][i])^2
  }
}
