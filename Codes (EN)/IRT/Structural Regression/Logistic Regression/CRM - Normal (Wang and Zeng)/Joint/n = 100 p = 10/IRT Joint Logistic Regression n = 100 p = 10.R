...
for (m in 1:r) {
  for (i in 1:n) {
    MSE_theta[m, i] = as.numeric((theta_estimates_mean[i, m]) - true_thetas[[1]][i])^2
  }
}
...
RMSEA_alpha = rep(0, p)
for (j in 1:p) {
  RMSEA_a[j] = sqrt(mean(MSE_a[, j]))
  RMSEA_b[j] = sqrt(mean(MSE_b[, j]))
  RMSEA_alpha[j] = sqrt(mean(MSE_alpha[, j]))
}

RMSEA_IRT = cbind(RMSEA_a, RMSEA_b, RMSEA_alpha)
